...
The core hypothesis of this project, shared with the companion CS229 study, is that
EEG's spatial blindness can be corrected by a **second modality with complementary
physics**: fNIRS measures the slow hemodynamic (metabolic) response via infrared light,
giving a far stronger indicator of *where* cortical activity occurs. Fusing fNIRS's
spatial fidelity with EEG's temporal fidelity should, in principle, synthesize the
"best of both worlds."

---

## 2. Background: the multimodal experiment (Ganapathy & Ahuja, CS229)

This repository is the engineering and agentic-iteration arm of a larger study,
*"Multimodal Semantic Neural Decoding with Combined EEG + fNIRS Recordings"*
(T. Ganapathy & N. Ahuja). That paper establishes the scientific result; this repo
implements, scales, iterates, and **deploys** the EEG learning system, and packages the
fNIRS fusion engine (`CHASEDOWN.py`).
...
iterates models autonomously?). The companion CS229 study supplied the hypothesis; this lab
...
- **AI disclosure.** This project leans deliberately into AI tooling, which is the point of
  ...
  numbers and the CS229 ablation study, not by model assertion.
...
- T. Ganapathy, N. Ahuja (2025). *Multimodal Semantic Neural Decoding with Combined EEG +
  fNIRS Recordings.* Stanford CS229.
